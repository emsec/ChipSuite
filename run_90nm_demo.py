...
from chipsuite import GDSLoader, StitchingInfo, BboxGenerator, PowerLineDetector2, Algorithm, Algorithm2
...
gdsloader = GDSLoader()
gdsloader.load_gds("data/90nm_demo/90nm.gds", IMAGE_EDGES, "R90", not BENCHMARK)



stitching_info = StitchingInfo()
stitching_info.load_mist_file("data/90nm_demo/img-global-positions-0.txt")
stitching_info.set_image_folder("data/90nm_demo/tiles")
...
if ALGORITHM == 2:
    ...
    algorithm.set_html_output("90nm_demo_fill_to_std.html")
    ...
    assert False, "This algorithm is not supported as there is no replacement in this demo."

algorithm.set_fract(8) # 1/n for the fused image

algorithm.set_interactive_mode(True, 0)

algorithm.set_output_folder("output")

algorithm.analyze(Algorithm.MODE_ONLY_CELLS+1 if ALGORITHM >= 3 else Algorithm.MODE_DEFAULT, starttime, (29, 10))



print(f"Completed. Benchmark time of 90nm DEMO Algorithm {ALGORITHM}: {time.process_time() - t}")
print(f"Real time of 90nm DEMO Algorithm {ALGORITHM}: {time.monotonic() - t2}")
